...
import { Plus, Users, Loader2, X, Crown, UserCheck, Mail } from 'lucide-react';
...
export default function TeamsPage() {
  ...
  // GSAP animation for grid layout
  useEffect(() => {
    if (isLoading || teams.length === 0 || activeTeamId) return;
    ...
    return () => ctx.revert();
  }, [isLoading, teams.length, activeTeamId]);
  ...
  // Handle back from chat workspace
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <TeamChat
              ...
              onPresenceUpdate={(ids) => setOnlineUserIds(ids)}
            />
          </div>
          ...
          <div className="space-y-6">
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-extrabold text-[#ededff] text-[15px]">Collaborators</h3>
                {/* Invite Member Button */}
                {activeTeam.members.find(
                  (m) => (typeof m.userId === 'object' ? m.userId?._id : m.userId) === user?._id
                )?.role === 'TeamLead' && (
                  <button
                    onClick={() => {
                      setSelectedTeamIdForInvite(activeTeam._id);
                      setIsInviteModalOpen(true);
                    }}
                    className="text-xs font-bold text-[#7c6fff] hover:text-[#a08cff] transition-colors"
                  >
                    + Add Member
                  </button>
                )}
              ...
              <div className="space-y-4.5">
                {activeTeam.members.map((m, mi) => {
                  const mUser = typeof m.userId === 'object' ? m.userId : null;
                  if (!mUser) return null;

                  const initials = mUser.name?.split(' ').map((n: string) => n[0]).slice(0, 2).join('').toUpperCase() ?? '?';
                  const isOnline = onlineUserIds.includes(mUser._id);
                  const isLead = m.role === 'TeamLead';

                  return (
                    <div
                      key={mi}
                      className="flex items-center justify-between p-3 rounded-2xl transition-colors duration-150"
                      style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}
                    >
                      <div className="flex items-center gap-3">
                        {/* Avatar bubble with Online glow dot */}
                        <div className="relative">
                          ...
                          <span
                            ...
                            style={{
                              borderColor: '#0d0d18',
                              background: isOnline ? '#10b981' : '#44445a',
                        ...
                        <div>
                          <p className="text-xs font-bold text-[#ededff] leading-none">{mUser.name}</p>
                          <p className="text-[10px] text-[#606080] mt-1 flex items-center gap-1">
                            <Mail size={10} /> {mUser.email}
                          </p>
                        </div>
                      </div>
                      ...
                      <span
                        ...
                        style={
                          isLead
                            ...
                            : { background: 'rgba(124,111,255,0.1)', color: '#a08cff' }
                        }
                      >
                        {isLead ? 'Lead' : 'Member'}
                      </span>
            ...
            >
              <h4 className="text-[13px] font-bold text-[#ededff] mb-2">About Workspace</h4>
              <p className="text-xs text-[#606080] leading-relaxed">
                {activeTeam.description || 'No workspace description provided.'}
              </p>
            </div>
          </div>
        </div>
      ) : (
        /* ─── Teams Grid List Mode ─── */
        <>
          {/* Header */}
